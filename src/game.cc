...
namespace {
  ...
  // of a real display.
  void
  DrawDigit(HDC hdc, int cell_x, int cell_y, int cell_w, int cell_h, int thickness, int digit) {
    ...
    auto seg_color           = [&](int bit) { return (mask & (1u << bit)) ? kSegOn : kSegOff; };
  ...
  int RacketX(int client_width, bool left_side) {
    return left_side ? kRacketEdgeMarginX : client_width - kRacketEdgeMarginX - kRacketW;
  }
  ...
  void TickPlayerRacket(HWND hWnd, float dt) {
    ...
    const bool up   = (GetAsyncKeyState(VK_UP) & 0x8000) || (GetAsyncKeyState(VK_LEFT) & 0x8000);
    const bool down = (GetAsyncKeyState(VK_DOWN) & 0x8000) || (GetAsyncKeyState(VK_RIGHT) & 0x8000);
    if (!up && !down) {
      return;
    }
  ...
  void TickMachineRacket(HWND hWnd, float dt) {
    ...
    float* racket_y            = machine_on_left ? &g_left_racket_y : &g_right_racket_y;
    ...
    const int read_idx    = (g_ai_history_idx - 1 - lag + kAiHistorySize) % kAiHistorySize;
  ...
  void ApplyRacketBounce(float ball_top_y, float racket_top_y, bool ball_now_moves_right) {
    ...
    const float speed  = std::sqrt(g_ball_dx * g_ball_dx + g_ball_dy * g_ball_dy);
...
void DrawMessageArea(HDC hdc, const RECT& client) {
  ...
  DrawTextW(hdc, text_to_render.c_str(), -1, &measure_rect, kDrawFlags | DT_CALCRECT);
...
void DrawRackets(HDC hdc, const RECT& client) {
  ...
  const COLORREF left_color  = g_player_on_left ? kPlayerRacketColor : kMachineRacketColor;
  const COLORREF right_color = g_player_on_left ? kMachineRacketColor : kPlayerRacketColor;
...
float NextFrameDelta() {
  ...
  const long long ticks = static_cast<long long>(now.QuadPart - g_qpc_last.QuadPart);
  g_qpc_last            = now;
  if (g_qpc_freq.QuadPart <= 0) {
    return 0.0f;
  }
  float dt = static_cast<float>(ticks) / static_cast<float>(g_qpc_freq.QuadPart);
  if (dt < 0.0f) {
    dt = 0.0f;
...
void TickBall(HWND hWnd, float dt) {
  ...
  if (off_left || off_right) {
    ...
    const unsigned int next_score = player_scored ? g_player_score + 1 : g_machine_score + 1;
    ...
    if (next_score >= kWonRoundPoints) {
      PostMessageW(hWnd, player_scored ? WM_APP_ROUND_WON : WM_APP_GAME_LOST, 0, 0);
    }
    return;
  }
...
bool ConfirmNewGame(HWND hWnd) {
  const int new_game_dialog =
      MessageBoxW(hWnd, L"Are you sure you want to start a new game?", L"Confirm New Game",
